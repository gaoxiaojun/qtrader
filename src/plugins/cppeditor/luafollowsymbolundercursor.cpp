...
#include "luaeditor.h"
#include "luavirtualfunctionassistprovider.h"
...
LuaEditorWidget::Link attemptFuncDeclDef(const QTextCursor &cursor,
    LuaEditorWidget *, CPlusPlus::Snapshot snapshot, const CPlusPlus::Document::Ptr &document,
    SymbolFinder *symbolFinder)
{
    ...
    if (target) {
        result = LuaEditorWidget::linkToSymbol(target);
...
FollowSymbolUnderCursor::FollowSymbolUnderCursor(LuaEditorWidget *widget)
    : m_widget(widget)
    ...
    SymbolFinder *symbolFinder, bool inNextSplit)
{
    ...
    if (macro) {
        ...
        const QByteArray name = LuaEditorWidget::identifierUnderCursor(&macroCursor).toLatin1();
        if (macro->name() == name)
