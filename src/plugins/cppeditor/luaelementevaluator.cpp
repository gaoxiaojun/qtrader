...
LuaElementEvaluator::LuaElementEvaluator(LuaEditorWidget *editor) :
    m_editor(editor),
...
void LuaElementEvaluator::setTextCursor(const QTextCursor &tc)
{ m_tc = tc; }

void LuaElementEvaluator::setLookupBaseClasses(const bool lookup)
{ m_lookupBaseClasses = lookup; }

void LuaElementEvaluator::setLookupDerivedClasses(const bool lookup)
{ m_lookupDerivedClasses = lookup; }

// @todo: Consider refactoring code from CPPEditor::findLinkAt into here.
void LuaElementEvaluator::execute()
{
    clear();
...
void LuaElementEvaluator::checkDiagnosticMessage(int pos)
{
...
bool LuaElementEvaluator::matchIncludeFile(const CPlusPlus::Document::Ptr &document, unsigned line)
{
    foreach (const Document::Include &includeFile, document->resolvedIncludes()) {
        if (includeFile.line() == line) {
            m_element = QSharedPointer<LuaElement>(new CppInclude(includeFile));
            return true;
        }
    }
    return false;
...
bool LuaElementEvaluator::matchMacroInUse(const CPlusPlus::Document::Ptr &document, unsigned pos)
{
    foreach (const Document::MacroUse &use, document->macroUses()) {
        if (use.contains(pos)) {
            const unsigned begin = use.begin();
            if (pos < begin + use.macro().name().length()) {
                m_element = QSharedPointer<LuaElement>(new CppMacro(use.macro()));
                return true;
            }
        }
    }
...
void LuaElementEvaluator::handleLookupItemMatch(const Snapshot &snapshot,
                                                ...
                                                const Scope *scope)
{
    ...
    if (!declaration) {
        ...
        m_element = QSharedPointer<LuaElement>(new Unknown(type));
    } else {
        const FullySpecifiedType &type = declaration->type();
        if (declaration->isNamespace()) {
            m_element = QSharedPointer<LuaElement>(new CppNamespace(declaration));
            ...
            m_element = QSharedPointer<LuaElement>(cppClass);
        } else if (Enum *enumDecl = declaration->asEnum()) {
            m_element = QSharedPointer<LuaElement>(new CppEnum(enumDecl));
        } else if (EnumeratorDeclaration *enumerator = dynamic_cast<EnumeratorDeclaration *>(declaration)) {
            m_element = QSharedPointer<LuaElement>(new CppEnumerator(enumerator));
        } else if (declaration->isTypedef()) {
            m_element = QSharedPointer<LuaElement>(new CppTypedef(declaration));
        } else if (declaration->isFunction()
                   || (type.isValid() && type->isFunctionType())
                   || declaration->isTemplate()) {
            m_element = QSharedPointer<LuaElement>(new LuaFunction(declaration));
        } else if (declaration->isDeclaration() && type.isValid()) {
            m_element = QSharedPointer<LuaElement>(
                new CppVariable(declaration, context, lookupItem.scope()));
        } else {
            m_element = QSharedPointer<LuaElement>(new CppDeclarableElement(declaration));
        }
    }
}

bool LuaElementEvaluator::identifiedCppElement() const
{
    return !m_element.isNull();
}

const QSharedPointer<LuaElement> &LuaElementEvaluator::cppElement() const
{
    return m_element;
}

bool LuaElementEvaluator::hasDiagnosis() const
{
    return !m_diagnosis.isEmpty();
}

const QString &LuaElementEvaluator::diagnosis() const
{
    return m_diagnosis;
}
...
// CppElement
LuaElement::LuaElement() : helpCategory(TextEditor::HelpItem::Unknown)
{}
    ...
    fileName(QFileInfo(includeFile.resolvedFileName()).fileName())
{
    ...
    helpMark = fileName;
    link = LuaEditorWidget::Link(path);
    tooltip = path;
}
...
CppMacro::CppMacro(const Macro &macro)
{
    ...
    helpMark = macroName;
    link = LuaEditorWidget::Link(macro.fileName(), macro.line());
    ...
    , icon(Icons().iconForSymbol(declaration))
{
    ...
    link = LuaEditorWidget::linkToSymbol(declaration);
    helpMark = name;
}
...
// CppFunction
LuaFunction::LuaFunction(Symbol *declaration)
