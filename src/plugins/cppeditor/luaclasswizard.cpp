...
LuaClassWizardDialog::LuaClassWizardDialog(QWidget *parent) :
    Utils::Wizard(parent),
...
void LuaClassWizardDialog::setPath(const QString &path)
{
    m_classNamePage->newClassWidget()->setPath(path);
}

LuaClassWizardParameters  LuaClassWizardDialog::parameters() const
{
    LuaClassWizardParameters rc;
...
LuaClassWizard::LuaClassWizard()
{
}

QString LuaClassWizard::sourceSuffix() const
{
    return preferredSuffix(QLatin1String(Constants::CPP_SOURCE_MIMETYPE));
}

QString LuaClassWizard::headerSuffix() const
{
    return preferredSuffix(QLatin1String(Constants::CPP_HEADER_MIMETYPE));
}

QWizard *LuaClassWizard::createWizardDialog(QWidget *parent,
                                            const Core::WizardDialogParameters &wizardDialogParameters) const
{
    LuaClassWizardDialog *wizard = new LuaClassWizardDialog(parent);
...
Core::GeneratedFiles LuaClassWizard::generateFiles(const QWizard *w, QString *errorMessage) const
{
    const LuaClassWizardDialog *wizard = qobject_cast<const LuaClassWizardDialog *>(w);
    const LuaClassWizardParameters params = wizard->parameters();
...
bool LuaClassWizard::generateHeaderAndSource(const LuaClassWizardParameters &params,
