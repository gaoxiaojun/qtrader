...
#include "luafollowsymbolundercursor.h"
#include "luahighlighter.h"
#include "luapreprocessordialog.h"
...
{
    LuaEditorWidget *editor;
    TypeOfExpression typeOfExpression;
    SemanticInfo info;

    CanonicalSymbol(LuaEditorWidget *editor, const SemanticInfo &info)
    ...
    static Scope *getScopeAndExpression(LuaEditorWidget *editor, const SemanticInfo &info,
...
LuaEditorWidget::LuaEditorWidget(QWidget *parent)
    ...
    , m_preprocessorButton(0)
{
    ...
    setAutoCompleter(new LuaAutoCompleter);

    baseTextDocument()->setSyntaxHighlighter(new LuaHighlighter);
...
{
    LuaEditor *editable = new LuaEditor(this);
    createToolBar(editable);
    return editable;
}

void LuaEditorWidget::createToolBar(LuaEditor *editor)
{
    ...
    if (LuaEditorPlugin::instance()->sortedOutline())
    ...
    connect(m_sortAction, SIGNAL(toggled(bool)),
            LuaEditorPlugin::instance(), SLOT(setSortedOutline(bool)));
...
void LuaEditorWidget::selectAll()
{
...
void LuaEditorWidget::setMimeType(const QString &mt)
{
...
void LuaEditorWidget::setObjCEnabled(bool onoff)
{
    m_objcEnabled = onoff;
}

bool LuaEditorWidget::isObjCEnabled() const
{ return m_objcEnabled; }

void LuaEditorWidget::startRename()
{
    m_inRenameChanged = false;
}

void LuaEditorWidget::finishRename()
{
...
void LuaEditorWidget::abortRename()
{
    if (m_currentRenameSelection <= NoCurrentRenameSelection)
        return;
...
void LuaEditorWidget::onDocumentUpdated()
{
    m_updateOutlineTimer->start();
}

const Macro *LuaEditorWidget::findCanonicalMacro(const QTextCursor &cursor, Document::Ptr doc) const
{
    if (!doc)
        return 0;
...
void LuaEditorWidget::findUsages()
{
    if (!m_modelManager)
        return;
...
void LuaEditorWidget::renameUsagesNow(const QString &replacement)
{
    if (!m_modelManager)
        return;
...
void LuaEditorWidget::renameUsages()
{
    renameUsagesNow();
}

void LuaEditorWidget::markSymbolsNow()
{
...
void LuaEditorWidget::markSymbols(const QTextCursor &tc, const SemanticInfo &info)
{
    abortRename();
...
void LuaEditorWidget::renameSymbolUnderCursor()
{
    if (!m_modelManager)
        return;
...
void LuaEditorWidget::onContentsChanged(int position, int charsRemoved, int charsAdded)
{
...
void LuaEditorWidget::updatePreprocessorButtonTooltip()
{
...
void LuaEditorWidget::jumpToOutlineElement(int index)
{
...
void LuaEditorWidget::setSortedOutline(bool sort)
{
...
bool LuaEditorWidget::sortedOutline() const
{
    return (m_proxyModel->sortColumn() == 0);
}

void LuaEditorWidget::updateOutlineNow()
{
    if (!m_modelManager)
        return;
...
void LuaEditorWidget::updateOutlineIndex()
{
    m_updateOutlineIndexTimer->start();
}

void LuaEditorWidget::highlightUses(const QList<SemanticInfo::Use> &uses,
                                    QList<QTextEdit::ExtraSelection> *selections)
{
    bool isUnused = false;
...
void LuaEditorWidget::updateOutlineIndexNow()
{
...
void LuaEditorWidget::updateOutlineToolTip()
{
    m_outlineCombo->setToolTip(m_outlineCombo->currentText());
}

void LuaEditorWidget::updateUses()
{
...
void LuaEditorWidget::updateUsesNow()
{
    if (m_currentRenameSelection != NoCurrentRenameSelection)
        return;
...
void LuaEditorWidget::highlightSymbolUsages(int from, int to)
{
    if (editorRevision() != m_highlightRevision)
        return; // outdated
...
void LuaEditorWidget::finishHighlightSymbolUsages()
{
    if (editorRevision() != m_highlightRevision)
        return; // outdated
...
void LuaEditorWidget::switchDeclarationDefinition(bool inNextSplit)
{
    if (!m_modelManager)
        return;
    ...
    LuaEditorWidget::Link symbolLink;
...
QString LuaEditorWidget::identifierUnderCursor(QTextCursor *macroCursor)
{
...
LuaEditorWidget::Link LuaEditorWidget::findLinkAt(const QTextCursor &cursor, bool resolveTarget,
                                                  bool inNextSplit)
{
    if (!m_modelManager)
        return Link();
...
unsigned LuaEditorWidget::editorRevision() const
{
    return document()->revision();
}

bool LuaEditorWidget::isOutdated() const
{
...
SemanticInfo LuaEditorWidget::semanticInfo() const
{
    return m_lastSemanticInfo;
}

CPlusPlus::OverviewModel *LuaEditorWidget::outlineModel() const
{
    return m_outlineModel;
}

QModelIndex LuaEditorWidget::outlineModelIndex()
{
...
bool LuaEditorWidget::event(QEvent *e)
{
    switch (e->type()) {
...
void LuaEditorWidget::performQuickFix(int index)
{
    ...
    if (!isOutdated()) {
        ...
        if (interface) {
            QScopedPointer<TextEditor::IAssistProcessor> processor(
                        LuaEditorPlugin::instance()->quickFixProvider()->createProcessor());
...
void LuaEditorWidget::keyPressEvent(QKeyEvent *e)
{
    if (m_currentRenameSelection == NoCurrentRenameSelection) {
...
{
    LuaEditorWidget *newEditor = new LuaEditorWidget(parent);
    ...
    LuaEditorPlugin::instance()->initializeEditor(newEditor);
...
bool LuaEditor::open(QString *errorString, const QString &fileName, const QString &realFileName)
{
...
const Utils::CommentDefinition *LuaEditor::commentDefinition() const
{
    return &m_commentDefinition;
}

TextEditor::CompletionAssistProvider *LuaEditor::completionAssistProvider()
{
    return CppModelManagerInterface::instance()->cppEditorSupport(this)->completionAssistProvider();
}

void LuaEditorWidget::setFontSettings(const TextEditor::FontSettings &fs)
{
...
void LuaEditorWidget::setTabSettings(const TextEditor::TabSettings &ts)
{
...
void LuaEditorWidget::slotCodeStyleSettingsChanged(const QVariant &)
{
    CppTools::QtStyleCodeFormatter formatter;
    formatter.invalidateCache(document());
}

LuaEditorWidget::Link LuaEditorWidget::linkToSymbol(CPlusPlus::Symbol *symbol)
{
    if (!symbol)
        return Link();
...
bool LuaEditorWidget::openCppEditorAt(const Link &link, bool inNextSplit)
{
...
void LuaEditorWidget::semanticRehighlight(bool force)
{
    if (m_modelManager)
        m_modelManager->cppEditorSupport(editor())->recalculateSemanticInfoDetached(force);
}

void LuaEditorWidget::highlighterStarted(QFuture<TextEditor::HighlightingResult> *highlighter,
                                         unsigned revision)
{
...
void LuaEditorWidget::updateSemanticInfo(const SemanticInfo &semanticInfo)
{
...
QModelIndex LuaEditorWidget::indexForPosition(int line, int column,
...
QVector<TextEditor::TextStyle> LuaEditorWidget::highlighterFormatCategories()
{
...
TextEditor::IAssistInterface *LuaEditorWidget::createAssistInterface(
    ...
    TextEditor::AssistReason reason) const
{
    if (kind == TextEditor::Completion) {
        ...
        if (!semanticInfo().doc || isOutdated())
            return 0;
        return new LuaQuickFixAssistInterface(const_cast<LuaEditorWidget *>(this), reason);
    } else {
...
QSharedPointer<FunctionDeclDefLink> LuaEditorWidget::declDefLink() const
{
    return m_declDefLink;
}

void LuaEditorWidget::onRefactorMarkerClicked(const TextEditor::RefactorMarker &marker)
{
    if (marker.data.canConvert<FunctionDeclDefLink::Marker>())
        applyDeclDefLinkChanges(true);
}

void LuaEditorWidget::updateFunctionDeclDefLink()
{
...
void LuaEditorWidget::updateFunctionDeclDefLinkNow()
{
...
void LuaEditorWidget::onFunctionDeclDefLinkFound(QSharedPointer<FunctionDeclDefLink> link)
{
    abortDeclDefLink();
    m_declDefLink = link;
...
void LuaEditorWidget::applyDeclDefLinkChanges(bool jumpToMatch)
{
    if (!m_declDefLink)
        return;
...
void LuaEditorWidget::updateContentsChangedSignal()
{
    connect(document(), SIGNAL(contentsChange(int,int,int)),
            this, SLOT(onContentsChanged(int,int,int)));
}

FollowSymbolUnderCursor *LuaEditorWidget::followSymbolUnderCursorDelegate()
{
    return m_followSymbolUnderCursor.data();
}

void LuaEditorWidget::abortDeclDefLink()
{
    if (!m_declDefLink)
        return;
...
bool LuaEditorWidget::handleDocumentationComment(QKeyEvent *e)
{
...
bool LuaEditorWidget::isStartOfDoxygenComment(const QTextCursor &cursor) const
{
    const int pos = cursor.position();
...
void LuaEditorWidget::onCommentsSettingsChanged(const CppTools::CommentsSettings &settings)
{
    m_commentsSettings = settings;
}

void LuaEditorWidget::showPreProcessorWidget()
{
